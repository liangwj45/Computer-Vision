static void icvHoughCirclesGradient(CvMat* img, float dp, float min_dist,
  ...
  / 确保累加器矩阵的分辨率不小于1 if (dp < 1.f) dp = 1.f;
  //分辨率的倒数
  idp = 1.f / dp;
  //根据分辨率，创建累加器矩阵
  accum = cvCreateMat(cvCeil(img->rows * idp) + 2, cvCeil(img->cols * idp) + 2,
                      CV_32SC1);
  //初始化累加器为0
  cvZero(accum);
  //创建两个序列，
  ...
  / 累加器的步长
    ...
    for (x = 0; x < cols; x++) {
      ...
      if (!edges_row[x] || (vx == 0 && vy == 0)) continue;
      //计算当前点的梯度值
  ...
  for (i = 0; i < centers->total; i++) {
    ...
    //如果j <
    // circles->total，说明当前的圆心已被认为与之前确定作为输出的圆心是同一个圆心，则抛弃该圆心，返回上面的for循环
    if (j < circles->total) continue;
    ...
    if (max_count > acc_threshold) {
      ...
      //如果得到的圆大于阈值，则退出该函数
      if (circles->total > circles_max) return;
    }
  }
}